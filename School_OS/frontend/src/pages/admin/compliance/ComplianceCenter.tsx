...
export default function ComplianceCenter() {
  ...
  const handleExportMinistryReport = async () => {
    try {
      addToast('Preparing ministry report export...', 'info');
      ...
      addToast('Compliance report ready for download.', 'success');
    } catch (err) {
      addToast('Failed to generate ministry report.', 'error');
    }
  };

  const handleRecalculateCompliance = async () => {
    try {
      addToast('Initiating compliance recalibration...', 'info');
      const response = await api.post('/gov/recalculate/');
      addToast(response.data.message || 'Compliance scores recalculated.', 'success');
    } catch (err) {
      addToast('Failed to recalculate scores.', 'error');
    }
  };
  ...
  return (
    <div className="p-4 lg:p-12 space-y-12 max-w-[1600px] mx-auto">
      ...
      <div className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-outline-variant/15 pb-8">
        <div>
          <span className="text-[0.65rem] font-bold uppercase tracking-[0.2em] text-primary/60 mb-2 block font-headline">MINESEC & Inspections</span>
          <h2 className="text-4xl font-semibold tracking-tight text-on-surface font-headline">Inspections & Ministry Reports</h2>
          <p className="text-on-surface-variant mt-2 max-w-xl text-lg leading-relaxed">
            Prepare for MINESEC inspections, manage compliance reports, and ensure alignment with national education standards.
          </p>
        </div>
      ...
      <footer className="mt-20 py-16 border-t border-outline-variant/15 text-center">
        <div className="max-w-3xl mx-auto space-y-6">
          ...
          <p className="text-on-surface-variant italic font-serif text-xl leading-relaxed">
            "Institutional integrity is not a destination, but a continuous process of accurate documentation and radical transparency."
          </p>
          <div className="flex flex-col items-center gap-2">
            <p className="text-[10px] font-black uppercase tracking-[0.5em] text-primary/40">- School OS Governance Charter v2.4</p>
