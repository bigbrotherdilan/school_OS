  ...
  return (
    <div className="p-4 lg:p-12 space-y-12 max-w-[1500px] mx-auto bg-white min-h-screen">
      <section className="flex flex-col md:flex-row justify-between items-end gap-6 border-b border-slate-100 pb-12">
        <div>
          <span className="text-[0.65rem] font-black uppercase tracking-[0.4em] text-primary/40 mb-3 block">Activity Records</span>
          <h1 className="text-4xl font-black tracking-tighter text-slate-900 uppercase">Activity Log</h1>
          <p className="text-slate-500 mt-4 max-w-xl text-lg font-medium leading-relaxed">
            Record of all administrative actions within the school system for transparency and accountability.
          </p>
        </div>
      ...
      <section className="space-y-8 bg-slate-50 p-10 rounded-[3rem] border border-slate-100 shadow-sm">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-8">
          <div className="relative w-full md:w-96 group">
            ...
            <input
              ...
              placeholder="Search by actor, endpoint or description..."
        ...
        {loading ? (
          <div className="bg-white rounded-3xl p-16 text-center shadow-sm border border-slate-100">
            ...
            <p className="text-slate-400 font-medium">Loading audit ledger...</p>
          </div>
        ) : logs.length === 0 ? (
          <div className="bg-white rounded-3xl p-16 text-center shadow-sm border border-slate-100">
            <span className="material-symbols-outlined text-5xl text-slate-300 mb-4 block">history</span>
            <p className="text-slate-400 font-medium">No audit logs available.</p>
          </div>
        ) : (
              ...
              <tbody className="divide-y divide-slate-50">
                {logs.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50 transition-all group">
                    ...
                    <td className="px-8 py-6">
                      <span className="text-[10px] font-bold text-slate-500 uppercase">{MODULE_LABELS[row.module] || row.module}</span>
                    </td>
                    <td className="px-8 py-6">
      ...
      <footer className="py-24 text-center flex flex-col items-center gap-12">
        ...
        <p className="text-slate-400 italic font-serif text-xl max-w-2xl leading-relaxed opacity-60">
          "The greatest asset of an institution is not its ledger of wealth, but its ledger of radical honesty."
        </p>
      </footer>
    </div>
  );
