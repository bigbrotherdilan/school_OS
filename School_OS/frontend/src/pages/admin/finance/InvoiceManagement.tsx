...
export default function InvoiceManagement() {
  ...
  const handleGenerate = async () => {
    if (!genForm.due_date) return addToast(t('Due date is required.'), 'error');
    setGenLoading(true);
    try {
      if (genForm.mode === 'class') {
        if (!genForm.class_id) return addToast(t('Select a class.'), 'error');
        const res = await api.post('/finance/invoices/batch-generate/', {
          class_id: genForm.class_id,
          due_date: genForm.due_date,
        });
        addToast(`${t('Fees generated for the entire class!')}${res.data.skipped ? ` ${t('{{count}} student(s) skipped (already billed).', { count: res.data.skipped })}` : ''}`, 'success');
      } else {
        if (!genForm.student_id) return addToast(t('Select a student.'), 'error');
        await api.post('/finance/invoices/generate/', {
          student_id: genForm.student_id,
          due_date: genForm.due_date,
        });
        addToast(t('Fee created for the student. Ready to share with the parent.'), 'success');
      }
      const res = await api.get('/finance/invoices/');
      setInvoices(res.data.results || res.data);
    } catch (err: any) {
      addToast(err.response?.data?.detail || t('Generation failed.'), 'error');
    } finally {
      setGenLoading(false);
    }
  };
  ...
  const handleSendReminder = async (invoiceId: string) => {
    ...
    try {
      ...
      addToast(res.data.detail || t('Reminder sent to parents.'), 'success');
    } catch (err: any) {
  ...
  const handleStatement = async (inv: any) => {
    try {
      ...
      addToast(err.response?.data?.detail || t('Failed to open statement.'), 'error');
    }
  };
  ...
  return (
    <div className="p-4 lg:p-12 space-y-8 max-w-[1400px] mx-auto animate-in fade-in duration-500">
      <div>
        <span className="text-primary font-bold tracking-widest text-xs uppercase mb-2 block">{t('Fees')}</span>
        <h1 className="text-4xl font-semibold tracking-tight text-on-surface">{t('Fees')}</h1>
        <p className="text-on-surface-variant mt-1">{t('View, generate, and manage student fees.')}</p>
      </div>

      <div className="flex gap-2 border-b border-outline-variant/10">
        <button onClick={() => setTab('list')} className={`px-6 py-3 text-xs font-bold uppercase tracking-widest transition-all ${tab === 'list' ? 'text-primary border-b-2 border-primary' : 'text-on-surface-variant hover:text-on-surface'}`}>{t('All Fees ({{num}})', { num: invoices.length })}</button>
        {canRecord && (
          <button onClick={() => setTab('generate')} className={`px-6 py-3 text-xs font-bold uppercase tracking-widest transition-all ${tab === 'generate' ? 'text-primary border-b-2 border-primary' : 'text-on-surface-variant hover:text-on-surface'}`}>{t('Generate Fees')}</button>
        )}
      </div>

      {tab === 'generate' && !canRecord && <div className="bg-amber-50 border border-amber-200 text-amber-800 rounded-2xl px-5 py-4 text-sm font-semibold">{t('Fee generation is restricted to the bursar for this school.')}</div>}

      {tab === 'generate' && (
        <div className="bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant/10 max-w-xl">
          <h3 className="font-bold text-on-surface mb-4">{t('Generate New Fees')}</h3>
          <div className="space-y-4">
            <div>
              <label className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant block mb-1">{t('Mode')}</label>
              <div className="flex gap-4">
                <label className="flex items-center gap-2 text-sm font-medium"><input type="radio" checked={genForm.mode === 'class'} onChange={() => setGenForm({ ...genForm, mode: 'class' })} className="w-4 h-4" /> {t('Entire Class')}</label>
                <label className="flex items-center gap-2 text-sm font-medium"><input type="radio" checked={genForm.mode === 'single'} onChange={() => setGenForm({ ...genForm, mode: 'single' })} className="w-4 h-4" /> {t('Single Student')}</label>
              </div>
            </div>
            <div>
              <label className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant block mb-1">{t('Class')}</label>
            ...
            {genForm.mode === 'single' && (
              <div>
                <label className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant block mb-1">{t('Student')}</label>
            ...
            <div>
              <label className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant block mb-1">{t('Due Date')}</label>
            ...
            <button onClick={handleGenerate} disabled={genLoading} className="px-6 py-3 bg-primary text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:shadow-lg transition-all disabled:opacity-50">
              {genLoading ? t('Generating...') : t('Generate Fees')}
            </button>
          </div>
        </div>
      )}
        ...
        <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/10 overflow-hidden">
          {loading ? (
            ...
            <div className="p-12 text-center text-on-surface-variant">
              ...
              <p className="text-sm font-bold">{t('No fees yet. Set up your fee structure to start billing.')}</p>
            </div>
          ) : (
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant/50 bg-surface-container-low">
                  ...
                  <th className="px-6 py-4">{t('Due')}</th>
                ...
                {invoices.map((inv: any) => (
                  <>
                    <tr key={inv.id} className="hover:bg-surface-container-low/50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm font-bold">{inv.balance === '0.00' ? <span className="text-secondary">{t('Settled')}</span> : `CFA ${parseFloat(inv.balance).toLocaleString()}`}</td>
                      <td className="px-6 py-4"><span className={`px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest ${statusColor(inv.status)}`}>{inv.status === 'partial' ? t('Incomplete Payment') : inv.status}</span></td>
                      ...
                      <td className="px-6 py-4">
                        ...
                        >
                          ...
                          {expanded === inv.id ? t('Hide') : t('View')} ({inv.line_items?.length || 0})
                        </button>
                      </td>
                      <td className="px-6 py-4">
                        <button
                            ...
                            {inv.line_items?.length ? inv.line_items.map((li: any, idx: number) => (
                              <span key={idx} className="inline-flex items-center gap-2 px-3 py-1.5 rounded-xl bg-surface-container-lowest border border-outline-variant/10 text-xs font-bold">
                                ...
                                <span className={`px-2 py-0.5 rounded-full text-[9px] font-black uppercase tracking-widest ${li.is_mandatory ? 'bg-secondary/15 text-secondary' : 'bg-amber-100 text-amber-700'}`}>
                                  {li.is_mandatory ? t('Mandatory') : t('Optional')}
                                </span>
                                <span className="text-on-surface-variant font-black">CFA {parseFloat(li.amount).toLocaleString()}</span>
                              </span>
                            )) : <span className="text-xs text-on-surface-variant font-medium">{t('No line items on this fee.')}</span>}
                          </div>
                        </td>
                      </tr>
                    )}
