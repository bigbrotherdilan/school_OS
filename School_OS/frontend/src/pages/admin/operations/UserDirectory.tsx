...
export default function UserDirectory() {
  ...
  const handleToggleActive = async (userId: string, isActive: boolean, userName: string) => {
    try {
      ...
      addToast(`${userName} ${isActive ? 'deactivated' : 'activated'}.`, 'success');
      fetchUsers();
    } catch {
  ...
  const copyCredentials = async (user: any) => {
    ...
    const text = `School OS Login Credentials\n\nName: ${user.full_name}\nEmail: ${user.email}\nLogin: ${loginUrl}\n\nContact the school admin for your password.`;
    try {
  ...
  const shareViaWhatsApp = (user: any) => {
    const role = user.roles?.[0]?.role || 'admin';
    const loginUrl = `${window.location.origin}/login/${role === 'super_admin' || role === 'admin' ? 'admin' : role}`;
    const text = encodeURIComponent(`School OS Login\n\nName: ${user.full_name}\nEmail: ${user.email}\nLogin: ${loginUrl}\n\nContact the school admin for your password.`);
  ...
  const handleInvite = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast(error.response?.data?.detail || 'Failed to create invitation.', 'error');
    } finally {
      setIsInviting(false);
    }
  };
  ...
  return (
    <div className="p-4 lg:p-12 space-y-12 max-w-[1600px] mx-auto animate-in fade-in duration-700">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          <span className="text-[10px] font-black uppercase tracking-[0.3em] text-primary/60 block mb-2">User Management</span>
          <h1 className="text-4xl font-black text-on-surface tracking-tight">User Directory</h1>
          <p className="text-on-surface-variant mt-2 text-lg font-medium max-w-2xl leading-relaxed">
            All user accounts across your school — administrators, teachers, bursars, and parents.
          </p>
        </div>
        <button
        ...
        <div className="flex gap-2 flex-wrap">
          {(['all', 'admin', 'teacher', 'bursar', 'parent'] as const).map(tab => (
            <button key={tab} onClick={() => setRoleFilter(tab)} className={`px-5 py-3 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all ${roleFilter === tab ? 'bg-primary text-white shadow-lg' : 'bg-surface-container-high text-on-surface-variant hover:bg-surface-container-highest'}`}>
              {tab === 'all' ? 'All Roles' : ROLE_LABELS[tab]?.label || tab}
            </button>
          ))}
        </div>
      </div>
      ...
      {isLoading ? (
        <div className="py-40 flex flex-col items-center justify-center gap-6">
          ...
          <p className="text-[10px] font-black uppercase tracking-[0.3em] text-on-surface-variant">Loading Users...</p>
        </div>
              ...
              <div key={user.id} className="grid grid-cols-12 gap-4 px-8 py-5 border-b border-outline-variant/5 hover:bg-surface-container-low/50 transition-colors items-center">
                ...
                <div className="col-span-1">
                  {user.is_active !== false ? (
                    <span className="px-2 py-1 bg-secondary/10 text-secondary text-[9px] font-black rounded-full uppercase tracking-widest">Active</span>
                  ) : (
                    <span className="px-2 py-1 bg-error/10 text-error text-[9px] font-black rounded-full uppercase tracking-widest">Inactive</span>
                  )}
                </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setShowInviteModal(false)}>
          <div className="bg-surface-container-lowest rounded-3xl w-full max-w-lg p-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="text-center mb-8">
              ...
              <h3 className="text-xl font-black text-on-surface mb-1">Invite User</h3>
              <p className="text-sm text-on-surface-variant">Send a shareable link for the user to set their own password.</p>
            </div>
            <form onSubmit={handleInvite} className="space-y-5">
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">First Name</label>
                  <input type="text" value={inviteForm.first_name} onChange={(e) => setInviteForm({ ...inviteForm, first_name: e.target.value })} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all" placeholder="Optional" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Last Name</label>
                  <input type="text" value={inviteForm.last_name} onChange={(e) => setInviteForm({ ...inviteForm, last_name: e.target.value })} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all" placeholder="Optional" />
                </div>
              </div>
              <div className="space-y-2">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4" onClick={() => setInviteResult(null)}>
          <div className="bg-surface-container-lowest rounded-3xl w-full max-w-md p-10 shadow-2xl animate-in fade-in zoom-in-95 duration-200" onClick={(e) => e.stopPropagation()}>
            <div className="text-center space-y-6">
              ...
              <div>
                <h3 className="text-xl font-black text-on-surface mb-1">Invitation Created</h3>
                <p className="text-sm text-on-surface-variant">Share this link with the user to set their password.</p>
              </div>
              ...
              <div className="flex gap-3">
                <button
                  onClick={() => {
                    ...
                    const text = encodeURIComponent(`You're invited to join School OS!\n\nClick the link to set your password:\n${link}`);
                    window.open(`https://wa.me/?text=${text}`, '_blank');
                  }}
      ...
      <footer className="mt-12 py-12 border-t border-outline-variant/10 text-center">
        <p className="text-[10px] font-black text-on-surface-variant/40 uppercase tracking-[0.5em] mb-4">User Management by School OS</p>
