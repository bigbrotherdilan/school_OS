...
export default function DisciplineAndTransfers() {
  ...
  const handleTransferAction = async () => {
    ...
    try {
      ...
      addToast(`Transfer ${status} successfully.`, 'success');
  ...
  const handleLogIncident = async () => {
    ...
    try {
      ...
      addToast('Incident recorded. The welfare team has been notified.', 'success');
  ...
  const handleCreateTransfer = async () => {
    ...
    try {
      ...
      addToast('Failed to create transfer request.', 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-4 lg:p-12 space-y-8 animate-in fade-in duration-500 max-w-[1600px] mx-auto">
      <div className="flex justify-between items-end">
        <div>
          <span className="text-error font-bold tracking-widest text-xs uppercase mb-2 block">Student Affairs</span>
          <h2 className="text-4xl font-semibold tracking-tight text-on-surface">Discipline & Transfers</h2>
          <p className="text-on-surface-variant text-lg mt-2">Manage student behavior records, disciplinary actions, and process out-of-school transfers.</p>
        </div>
      ...
      <div className="grid grid-cols-12 gap-8">
        ...
        <div className="col-span-12 xl:col-span-7 bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b border-outline-variant/15 flex justify-between items-center bg-surface-container-low/30">
            <h3 className="text-xl font-bold text-on-surface">Disciplinary Logbook</h3>
            <div className="flex bg-surface-container-high rounded-lg p-1">
              <button className="px-4 py-1.5 text-sm font-bold bg-white text-on-surface rounded shadow-sm">Recent</button>
              <button className="px-4 py-1.5 text-sm font-bold text-on-surface-variant hover:text-on-surface">Severe</button>
            </div>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex-1 p-16 flex flex-col items-center justify-center text-center">
              ...
              <h4 className="text-lg font-bold text-on-surface mb-2">Clean Record</h4>
              <p className="text-sm text-on-surface-variant mb-6 max-w-sm">No disciplinary incidents have been logged for the current academic session.</p>
            </div>
          ) : (
            <div className="p-0">
              <table className="w-full text-left">
                <thead>
                  <tr className="bg-surface-container text-outline text-[11px] font-bold uppercase tracking-wider">
                    ...
                    <th className="p-4 text-right pr-6">Severity</th>
                  </tr>
                </thead>
                  ...
                  {discipline.map((d: any) => (
                    <tr key={d.id} className="hover:bg-surface-container-low/50 transition-colors">
                      ...
                      <td className="p-4">
                        ...
                        <div className="text-[10px] text-on-surface-variant mt-1">Reported by: {d.reported_by_name || 'N/A'}</div>
                      </td>
        ...
        <div className="col-span-12 xl:col-span-5 bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm overflow-hidden flex flex-col">
          <div className="p-6 border-b border-outline-variant/15 flex justify-between items-center bg-surface-container-low/30">
            <h3 className="text-xl font-bold text-on-surface">Transfer Processing</h3>
          ...
          {loading ? (
            ...
            <div className="flex-1 p-16 flex flex-col items-center justify-center text-center">
              ...
              <h4 className="text-lg font-bold text-on-surface mb-2">No Active Transfers</h4>
              <p className="text-sm text-on-surface-variant">There are no pending student transfer requests.</p>
            </div>
          ) : (
            <div className="divide-y divide-outline-variant/10">
              {transfers.map((t: any) => (
                <div key={t.id} className="p-6 hover:bg-surface-container-low/50 transition-colors">
                  <div className="flex justify-between items-start mb-3">
                    <div className="font-bold text-on-surface">{t.student_name}</div>
                    <span className={`text-[10px] font-bold uppercase tracking-wider px-2 py-1 rounded ${t.status === 'pending' ? 'bg-secondary-container text-on-secondary-container' : t.status === 'approved' ? 'bg-primary/10 text-primary' : 'bg-error/10 text-error'}`}>
                      {t.status}
                    </span>
                  </div>
                  <div className="flex items-center gap-3 text-sm text-on-surface-variant mb-2">
                    <span>{t.from_school || 'Current School'}</span>
                    <span className="material-symbols-outlined text-sm">arrow_right_alt</span>
                    <span className="font-medium text-on-surface">{t.to_school || t.destination_school}</span>
                  </div>
                  {t.reason && <p className="text-xs text-on-surface-variant mb-4 italic">"{t.reason}"</p>}
                  {t.status === 'pending' && (
                    <div className="flex gap-2">
                      <button onClick={() => setTransferModal({ type: 'approve', item: t })} className="flex-1 bg-primary text-white py-2 rounded-lg text-sm font-semibold hover:opacity-90">Approve</button>
                      <button onClick={() => setTransferModal({ type: 'review', item: t })} className="flex-1 bg-surface-container-high text-on-surface py-2 rounded-lg text-sm font-semibold hover:bg-surface-container-highest">Review</button>
                    </div>
                  )}
                </div>
              ))}
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-surface-container-lowest w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className={`p-8 border-b border-outline-variant/10 flex justify-between items-center ${transferModal.type === 'approve' ? 'bg-primary' : 'bg-surface-variant'}`}>
              <div>
                <h3 className="text-2xl font-bold text-white">{transferModal.type === 'approve' ? 'Approve' : 'Reject'} Transfer</h3>
            ...
            <div className="p-8 space-y-4">
              <div className="bg-surface-container-low rounded-xl p-4 space-y-2">
                <div className="flex items-center gap-3 text-sm">
                  <span>{transferModal.item.from_school || 'Current School'}</span>
                  <span className="material-symbols-outlined text-sm">arrow_right_alt</span>
              ...
              <div className="flex gap-4 pt-4">
                <button onClick={() => setTransferModal(null)} className="flex-1 py-3 border border-outline-variant/30 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-surface-variant transition-all active:scale-95">Cancel</button>
                <button onClick={handleTransferAction} disabled={saving} className={`flex-1 py-3 text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:opacity-90 shadow-lg transition-all active:scale-95 disabled:opacity-50 ${transferModal.type === 'approve' ? 'bg-primary' : 'bg-surface-variant'}`}>
                  {saving ? 'Processing...' : transferModal.type === 'approve' ? 'Confirm Approval' : 'Confirm Rejection'}
                </button>
              </div>
            </div>
          </div>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-surface-container-lowest w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-outline-variant/10 flex justify-between items-center bg-error">
              <div>
                <h3 className="text-2xl font-bold text-white">Log Incident</h3>
                <p className="text-red-100 text-sm">Record a new disciplinary event</p>
              </div>
            ...
            <div className="p-8 space-y-6">
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Action Taken</label>
                <textarea
              ...
              <div className="flex gap-4 pt-4">
                <button onClick={() => setIncidentModal(false)} className="flex-1 py-3 border border-outline-variant/30 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-surface-variant transition-all active:scale-95">Cancel</button>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-surface-container-lowest w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-outline-variant/10 flex justify-between items-center bg-primary">
              <div>
                <h3 className="text-2xl font-bold text-white">New Transfer Request</h3>
                <p className="text-blue-100 text-sm">Submit a student transfer to another school</p>
              </div>
            ...
            <div className="p-8 space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Destination School *</label>
                <input
                  type="text"
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Reason *</label>
                <textarea
              ...
              <div className="flex gap-4 pt-4">
                <button onClick={() => setNewTransferModal(false)} className="flex-1 py-3 border border-outline-variant/30 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-surface-variant transition-all active:scale-95">Cancel</button>
