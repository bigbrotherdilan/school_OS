...
export default function AddParentPage() {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setResult(res.data);
      addToast(t('Parent {{name}} created.', { name: formData.first_name }), 'success');
    } catch (error: any) {
      const data = error.response?.data;
      let detail = t('Failed to create parent.');
  ...
  const handleLink = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast(t('{{name}} linked to student.', { name: selectedParent.full_name }), 'success');
      setSelectedParent(null);
      setLinkStudentId('');
      setSearchQuery('');
      setSearchResults([]);
    } catch (error: any) {
      addToast(error.response?.data?.detail || t('Failed to link parent.'), 'error');
    } finally {
      setIsSubmitting(false);
    }
  };
    ...
    return (
      <div className="p-4 lg:p-12 max-w-[1000px] mx-auto bg-surface min-h-screen">
        ...
        <div className="bg-surface-container-lowest p-12 rounded-3xl border border-outline-variant/10 shadow-sm max-w-lg mx-auto text-center space-y-8">
          ...
          <div>
            <h2 className="text-2xl font-black text-on-surface mb-2">{t('Parent Account Created')}</h2>
            ...
            {result.linked_students && result.linked_students.length > 0 && (
              <p className="text-xs font-bold text-success mt-2">
                {t('Linked to {{count}} student(s) in {{school}}.', { count: result.linked_students.length, school: result.user.home_school })}
              </p>
            )}
          </div>
          ...
          <div className="bg-surface-container-low p-6 rounded-2xl space-y-3">
            ...
            <div className="flex justify-between text-sm">
              <span className="text-on-surface-variant">{t('Home School')}</span>
              <span className="font-bold text-on-surface">{result.user.home_school}</span>
            </div>
          </div>

          {result.temp_password && (
            <CredentialsCard email={result.user.email} password={result.temp_password} label={t('Parent Temporary Password')} loginPortal="parent" note={t('This parent is global: they can be linked to children at any other school. This password is shown only once — share it in person or by phone.')} />
          )}
  ...
  return (
    <div className="p-4 lg:p-12 max-w-[1200px] mx-auto bg-surface min-h-screen">
      ...
      <section className="mb-10">
        <span className="text-[10px] font-black uppercase tracking-[0.3em] text-teal-600/80 block mb-3">{t('Family Hub')}</span>
        <h1 className="text-4xl font-black tracking-tight text-on-surface">{t('Manage Parents')}</h1>
        <p className="text-on-surface-variant mt-2 text-lg leading-relaxed max-w-3xl">
          {t('Parents are global accounts — they are not tied to a school. Create one, give them the temporary password, and any school can link them to a child.')}
        </p>
      </section>
        ...
        <form onSubmit={handleCreate} className="bg-surface-container-lowest p-10 rounded-3xl border border-outline-variant/10 shadow-sm space-y-8 max-w-4xl">
          <div className="space-y-6">
            <h3 className="text-xl font-bold tracking-tight flex items-center gap-3 border-b border-outline-variant/10 pb-4">
              <UserCircle className="text-teal-600 w-6 h-6" /> {t('Parent Details')}
            </h3>
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                ...
                <input required type="email" name="email" value={formData.email} onChange={handleChange} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all placeholder:text-on-surface-variant/40" placeholder={t('e.g. parent@email.com')} />
              </div>
              <div className="space-y-2">
            ...
            <div className="grid grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Preferred Language')}</label>
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Relationship to Selected Students')}</label>
                <select value={relationshipType} onChange={e => setRelationshipType(e.target.value)} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all">
                  {RELATIONSHIP_TYPES.map(rel => <option key={rel.value} value={rel.value}>{t(rel.label)}</option>)}
                </select>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-4">
            <h3 className="text-xl font-bold tracking-tight flex items-center gap-3 border-b border-outline-variant/10 pb-4">
              <Users className="text-teal-600 w-6 h-6" /> {t('Link Students (optional, this school only)')}
            </h3>
            {students.length === 0 ? (
              <p className="text-sm text-on-surface-variant">{t('No students yet in this school. Register students first, then come back to link them.')}</p>
            ) : (
              <div className="grid grid-cols-2 md:grid-cols-3 gap-3 max-h-72 overflow-y-auto pr-2">
                {students.map((s: any) => (
                  ...
                  >
                    ...
                    <span className="block text-[10px] font-semibold text-on-surface-variant mt-1">{s.class_display || t('Unassigned')} · {s.admission_number || ''}</span>
                  </button>
                ))}
              </div>
            )}
          ...
          <div className="flex justify-end pt-6">
            <button disabled={isSubmitting || !formData.first_name || !formData.last_name || !formData.email} type="submit" className="flex items-center gap-3 px-8 py-4 bg-teal-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-teal-600/20 hover:shadow-xl active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
              ...
              {isSubmitting ? t('Creating Parent...') : t('Create Parent Account')}
            </button>
          </div>
        </form>
      ) : (
        <div className="bg-surface-container-lowest p-10 rounded-3xl border border-outline-variant/10 shadow-sm space-y-8 max-w-4xl">
          <div className="space-y-2">
            <h3 className="text-xl font-bold tracking-tight flex items-center gap-3 pb-2">
              <Search className="text-teal-600 w-6 h-6" /> {t('Find an Existing Parent')}
            </h3>
            <p className="text-sm text-on-surface-variant">{t('Search by email or name across all schools. The parent does not need any account at this school.')}</p>
            <div className="flex gap-3 pt-2">
              <input
                ...
                placeholder={t('e.g. parent@email.com or a name')}
              ...
              {searchResults.map((p: any) => (
                <div key={p.id} className={`flex items-center justify-between gap-4 p-4 rounded-xl border transition-all ${selectedParent?.id === p.id ? 'border-teal-600 bg-teal-600/10' : 'border-outline-variant/15 bg-surface-container-high'}`}>
                  ...
                  <div className="flex items-center gap-3">
                    <span className="text-[10px] font-bold text-on-surface-variant bg-surface-container-lowest px-3 py-1.5 rounded-lg">{t('{{count}} linked student(s)', { count: p.linked_students })}</span>
                    <button
                      type="button"
                    ...
                    >
                      {selectedParent?.id === p.id ? t('Selected') : t('Select')}
                    </button>
                  </div>
                </div>
              ))}
          ...
          {searchQuery && !isSubmitting && searchResults.length === 0 && (
            <p className="text-sm text-on-surface-variant">{t('No parents found. Try the')} <button type="button" className="text-teal-600 font-bold underline" onClick={() => setMode('create')}>{t('Create New Parent')}</button> {t('tab instead.')}</p>
          )}
          ...
          {selectedParent && (
            <form onSubmit={handleLink} className="border-t border-outline-variant/15 pt-8 space-y-6">
              <h3 className="text-lg font-black flex items-center gap-2 text-on-surface">
                <Link2 className="w-5 h-5 text-teal-600" /> {t('Link {{name}} to a Student', { name: selectedParent.full_name })}
              </h3>
              <div className="grid grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <select required value={linkStudentId} onChange={e => setLinkStudentId(e.target.value)} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all">
                    ...
                    {students.map((s: any) => (
                      <option key={s.id} value={s.id}>{s.full_name || `${s.first_name} ${s.last_name}`} — {s.class_display || t('Unassigned')}</option>
                    ))}
                  </select>
                </div>
                <div className="space-y-2">
              ...
              <div className="flex justify-end">
                <button disabled={isSubmitting || !linkStudentId} type="submit" className="flex items-center gap-3 px-8 py-4 bg-teal-600 text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg shadow-teal-600/20 hover:shadow-xl active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed">
                  <Link2 className="w-5 h-5" /> {t('Link Parent to Student')}
                </button>
              </div>
            </form>
          )}
