...
export default function Examinations() {
  ...
  const handleCreate = async () => {
    ...
    try {
      ...
      addToast(t('Examination scheduled. Students will be notified.'), 'success');
      setShowModal(false);
      setForm({ name: '', term: '', weight: '' });
      fetchData();
    } catch {
      addToast(t('Failed to create examination.'), 'error');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="p-4 lg:p-12 space-y-8 animate-in fade-in duration-500 max-w-[1600px] mx-auto">
      <div className="flex justify-between items-end">
        <div>
          <span className="text-primary font-bold tracking-widest text-xs uppercase mb-2 block">{t('Student Assessment')}</span>
          <h2 className="text-4xl font-semibold tracking-tight text-on-surface">{t('Examinations & Reporting')}</h2>
          <p className="text-on-surface-variant text-lg mt-2">{t('Configure exams, manage reporting windows, and generate official transcript records.')}</p>
        </div>
      ...
      <div className="grid grid-cols-12 gap-8">
        <div className="col-span-12 xl:col-span-8 bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm overflow-hidden">
          <div className="p-6 border-b border-outline-variant/15 flex justify-between items-center bg-surface-container-low/30">
            <h3 className="text-xl font-bold text-on-surface">{t('Registered Examinations')}</h3>
          ...
          {loading ? (
            <div className="p-12 text-center text-on-surface-variant">
              ...
              <p>{t('Loading assessment records...')}</p>
            </div>
          ) : exams.length === 0 ? (
            <div className="p-16 text-center">
              <span className="material-symbols-outlined text-4xl text-outline mb-4">assignment_add</span>
              <h4 className="text-lg font-bold text-on-surface mb-2">{t('No Exams Configured')}</h4>
              <p className="text-sm text-on-surface-variant mb-6">{t('Create the first examination entry to begin tracking sequence marks and final results.')}</p>
              <button onClick={() => setShowModal(true)} className="text-primary font-semibold hover:underline border border-primary/20 px-6 py-2 rounded-lg">{t('Create Exam')}</button>
            </div>
          ) : (
            <table className="w-full text-left">
              <thead>
                <tr className="bg-surface-container text-outline text-[11px] font-bold uppercase tracking-wider">
                  <th className="p-4 pl-6">{t('Exam Name')}</th>
                  <th className="p-4">{t('Academic Term')}</th>
                  <th className="p-4">{t('Weighting')}</th>
              ...
              <tbody className="divide-y divide-outline-variant/10">
                {exams.map((exam: any) => (
                  <tr key={exam.id} className="hover:bg-surface-container-low/50 transition-colors">
                    ...
                    <td className="p-4 text-sm text-on-surface-variant">{exam.term_name || t('Current Term')}</td>
                    ...
                    <td className="p-4 pr-6 text-right">
                      <button className="text-primary hover:underline text-sm font-semibold">{t('Manage')}</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="col-span-12 xl:col-span-4 space-y-6">
          <div className="bg-surface-container-lowest p-6 rounded-2xl border border-outline-variant/15 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <h3 className="font-bold text-on-surface">{t('Generate Reports')}</h3>
            </div>
            <p className="text-sm text-on-surface-variant mb-6">{t('Compile official termly report cards and class master sheets once marks are validated.')}</p>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-300">
          <div className="bg-surface-container-lowest w-full max-w-lg rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300">
            <div className="p-8 border-b border-outline-variant/10 flex justify-between items-center bg-primary">
              <div>
                <h3 className="text-2xl font-bold text-white">{t('New Examination')}</h3>
                <p className="text-blue-100 text-sm">{t('Configure a new exam entry for the current term')}</p>
              </div>
            ...
            <div className="p-8 space-y-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Exam Name *')}</label>
                <input
                  type="text"
                  value={form.name}
                  onChange={(e) => setForm({ ...form, name: e.target.value })}
                  placeholder={t('e.g. First Term Examination 2026')}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Term *')}</label>
                <select
                  value={form.term}
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Weight (%) *')}</label>
                <input
                  type="number"
                  min="0"
                  max="100"
              ...
              <div className="flex gap-4 pt-4">
                <button onClick={() => setShowModal(false)} className="flex-1 py-3 border border-outline-variant/30 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-surface-variant transition-all active:scale-95">{t('Cancel')}</button>
                <button onClick={handleCreate} disabled={saving} className="flex-1 py-3 bg-primary text-white rounded-xl font-bold text-xs uppercase tracking-widest hover:opacity-90 shadow-lg transition-all active:scale-95 disabled:opacity-50">
                  {saving ? t('Creating...') : t('Create Examination')}
                </button>
              </div>
            </div>
          </div>
