...
export default function Step4GenerateReview({ sectionId, yearId, onDone, onBack }: Props) {
  ...
  const handleCheck = async () => {
    ...
    try {
      ...
      addToast(err.response?.data?.detail || 'Feasibility check failed.', 'error');
    } finally {
      setChecking(false);
    }
  };
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      addToast(res.data.message || 'Timetables generated!', 'success');
    } catch (err: any) {
      const msg = err.response?.data?.detail || err.response?.data?.message || 'Generation failed.';
      addToast(msg, 'error');
    ...
    <div className="space-y-6">
      <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-lg font-bold">Step 4: Review & Generate</h3>
            <p className="text-sm text-on-surface-variant">Run a feasibility check first, then generate clash-free timetables for the whole section.</p>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          >
            ...
            {checking ? 'Checking feasibility…' : 'Run feasibility check'}
          </button>
            ...
            <div className="animate-in fade-in duration-300 space-y-3">
              ...
              <div className={`flex items-center gap-3 px-5 py-3.5 rounded-2xl border ${
                ...
                <div>
                  <p className={`font-bold text-sm ${checkResult.ready ? 'text-green-800' : 'text-red-800'}`}>
                    {checkResult.ready ? 'Ready to generate!' : 'Issues found — fix before generating.'}
                  </p>
                  <p className={`text-xs ${checkResult.ready ? 'text-green-700' : 'text-red-700'}`}>
                    {checkResult.count} issue{checkResult.count !== 1 ? 's' : ''} found
                    {checkResult.shared_teacher_count > 0 && ` · ${checkResult.shared_teacher_count} shared teacher(s) across sections`}
                  </p>
                </div>
              </div>
              ...
              {hasWarnings && !hasErrors && (
                <p className="text-xs text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-4 py-2.5">
                  ⚠ Some subjects have no assigned teacher and will appear as <strong>TBD</strong> on the timetable. You can still generate.
                </p>
              )}
            </div>
          )}
        ...
        <div className="mt-6 pt-6 border-t border-outline-variant/10">
          {genResult ? (
              ...
              {genResult.ok !== false ? (
                <>
                  <div className="flex items-center gap-3 mb-3">
                    ...
                    <p className="font-bold text-green-800">Timetables generated successfully!</p>
                  </div>
                    ...
                    <div className="mt-4 space-y-1.5">
                      {genResult.classes.map((c: any, i: number) => (
                        <div key={i} className="flex items-center justify-between bg-white/70 rounded-xl px-4 py-2 text-xs font-semibold text-green-800 border border-green-100">
                          ...
                          <span>{c.slots} slots · {c.status}</span>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            >
              {generating
                ? <><RefreshCw className="w-5 h-5 animate-spin" /> Solving — this may take a moment…</>
                : <><Sparkles className="w-5 h-5" /> Generate Timetables for Whole Section</>}
            </button>
          )}
          {hasErrors && !genResult && (
            <p className="text-xs text-red-600 text-center mt-2">Fix the errors above before generating.</p>
          )}
          {!checkResult && !genResult && (
            <p className="text-xs text-outline text-center mt-2">Run the feasibility check first (optional but recommended).</p>
          )}
        </div>
      </div>
