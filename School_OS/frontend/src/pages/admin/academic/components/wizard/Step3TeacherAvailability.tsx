...
export default function Step3TeacherAvailability({ sectionId, yearId, onNext, onBack }: Props) {
  ...
  const loadTeachers = async () => {
    ...
    try {
      ...
      addToast('Failed to load teacher availability data.', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      if (addForm.whole_day) {
        ...
        addToast(
          onDay.length
            ? `Day cleared — ${addModal.teacherName} is free all day.`
            : `${addModal.teacherName} is already free all day.`,
          'success'
        );
      } else {
        ...
        addToast('Unavailability block saved.', 'success');
      }
      setAddModal(null);
      loadTeachers();
    } catch (err: any) {
      addToast(err.response?.data?.detail || 'Failed to save block.', 'error');
    } finally {
      setAddSaving(false);
    }
  };
    ...
    <div className="space-y-6">
      <div className="bg-surface-container-lowest rounded-2xl border border-outline-variant/15 shadow-sm p-6">
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-lg font-bold">Step 3: Teacher Availability</h3>
            <p className="text-sm text-on-surface-variant">Block out times when teachers are unavailable — the generator will never use those slots.</p>
          </div>
        </div>

        {loading ? (
          <div className="py-16 text-center text-on-surface-variant animate-pulse">Loading teachers…</div>
        ) : teachers.length === 0 ? (
          <div className="py-12 text-center bg-surface-container-low rounded-2xl border border-outline-variant/10">
            ...
            <p className="font-semibold text-on-surface-variant text-sm">No teachers assigned yet.</p>
            <p className="text-xs text-outline mt-1">Link teachers to subjects in this section first, then return here.</p>
          </div>
        ) : (
                ...
                <div key={t.id} className="border border-outline-variant/15 rounded-2xl overflow-hidden hover:shadow-sm transition-shadow">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {t.windows?.length > 0 && (
                        <span className="text-[10px] font-black uppercase tracking-widest px-2.5 py-1 bg-amber-100 text-amber-700 rounded-full shrink-0">
                          {t.windows.length} block{t.windows.length > 1 ? 's' : ''}
                        </span>
                      )}
                    </div>
                  ...
                  {isOpen && (
                    <div className="p-4 bg-surface-container-lowest animate-in slide-in-from-top-1 duration-150 space-y-2">
                      {!t.windows?.length ? (
                        <p className="text-xs text-outline text-center py-4">No blocks set — teacher is fully available.</p>
                      ) : (
                        t.windows.map((w: any) => (
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              <div>
                                <p className="text-xs font-bold text-amber-800">
                                  {DAYS.find(d => d.value === w.day_of_week)?.label} · {w.start_time}–{w.end_time}
                                </p>
        ...
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-in fade-in duration-200">
          <div className="bg-surface-container-lowest w-full max-w-md rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 duration-200">
            <div className="p-6 border-b border-outline-variant/10 flex justify-between items-center">
              <div>
                <h3 className="text-lg font-bold">Block Unavailability</h3>
            ...
            <div className="p-6 space-y-4">
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Day</label>
              ...
              <label className="flex items-center gap-2.5 cursor-pointer select-none">
                ...
                />
                <span className="text-sm font-semibold">Free all day</span>
                <span className="text-[10px] text-on-surface-variant">
                  clears any blocks on this day — teacher can be scheduled anywhere
                </span>
              </label>
              <div>
                <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">Reason (optional)</label>
                <input type="text" placeholder="e.g. Other school, meetings" value={addForm.reason} onChange={e => setAddForm(f => ({ ...f, reason: e.target.value }))} className={`${inputCls} mt-1`} />
              </div>
              <div className="flex gap-4 pt-2">
                <button onClick={() => setAddModal(null)} className="flex-1 py-3 border border-outline-variant/30 rounded-xl font-bold text-xs uppercase tracking-widest hover:bg-surface-variant transition-all">Cancel</button>
                <button onClick={handleAdd} disabled={addSaving} className="flex-1 py-3 bg-primary text-white rounded-xl font-bold text-xs uppercase tracking-widest shadow-lg hover:opacity-90 transition-all disabled:opacity-50">
                  {addSaving ? 'Saving…' : addForm.whole_day ? 'Free Day' : 'Save Block'}
                </button>
              </div>
            </div>
          </div>
