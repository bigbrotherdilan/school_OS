...
export default function StudentPromotion() {
  ...
  const executePromotion = async () => {
    ...
    if (!window.confirm("ARE YOU SURE? This will globally move the selected students to their next classes. This action IS IRREVERSIBLE.")) return;
    
    setExecuting(true);
    try {
      await api.post('/students/students/promote/', { from_class: selectedClass, cutoff: cutoff });
      setSuccess("Promotion processed for the selected students!");
      setPreview([]);
    } catch (err) {
  ...
  return (
    <div className="p-4 lg:p-12 space-y-8 animate-in fade-in duration-500 max-w-[1600px] mx-auto">
      <div className="flex justify-between items-end">
        <div>
          <span className="text-tertiary font-bold tracking-widest text-xs uppercase mb-2 block tracking-wider">End of Year Process</span>
          <h2 className="text-4xl font-semibold tracking-tight text-on-surface">Class Promotion</h2>
          <p className="text-on-surface-variant text-lg mt-2">Manage the transition of students between class levels based on performance results.</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="bg-surface-container-lowest rounded-3xl border border-outline-variant/15 shadow-sm p-8 h-fit space-y-6">
          <div className="flex items-center gap-4 mb-2">
            ...
            <h3 className="text-xl font-bold text-on-surface">Promotion Logic</h3>
          </div>
          ...
          <div className="space-y-4">
            <div>
              <label className="text-sm font-bold text-on-surface-variant uppercase tracking-wider mb-2 block">Source Class</label>
              <select 
                value={selectedClass}
            ...
            <div>
              <label className="text-sm font-bold text-on-surface-variant uppercase tracking-wider mb-2 block">Minimum Average (0-20)</label>
          ...
          <div className="bg-warning-container/30 border border-warning/10 rounded-2xl p-4 flex gap-4">
             ...
             <p className="text-xs text-on-surface-variant leading-relaxed">
               <strong>Platform Guard:</strong> Ensure all final term exams are published before execution.
             </p>
          </div>
        ...
        <div className="lg:col-span-2 bg-surface-container-lowest rounded-3xl border border-outline-variant/15 shadow-sm overflow-hidden min-h-[600px]">
          ...
          {success ? (
             <div className="p-24 flex flex-col items-center justify-center text-center animate-in zoom-in duration-500">
               ...
               <h3 className="text-2xl font-bold text-on-surface mb-2">Success!</h3>
              ...
              <table className="w-full border-collapse">
                <thead>
                  <tr className="bg-surface-container-low/50">
                    <th className="text-left py-4 px-8 text-xs font-bold text-on-surface-variant uppercase tracking-widest">Student</th>
                    <th className="text-center py-4 px-8 text-xs font-bold text-on-surface-variant uppercase tracking-widest">Average</th>
                    <th className="text-right py-4 px-8 text-xs font-bold text-on-surface-variant uppercase tracking-widest">Decision</th>
                  </tr>
                </thead>
            ...
            <div className="flex flex-col items-center justify-center p-24 text-on-surface-variant text-center space-y-4 opacity-70">
              ...
              <div className="space-y-1">
                <h3 className="text-xl font-bold text-on-surface">Ready for Preview</h3>
                <p className="max-w-xs mx-auto text-sm leading-relaxed">Select a class and click preview to see automated promotion eligibility results.</p>
              </div>
            </div>
          )}
        </div>
