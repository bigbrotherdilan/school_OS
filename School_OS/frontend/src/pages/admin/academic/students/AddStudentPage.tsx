  ...
  return (
    <div className="p-4 lg:p-12 max-w-[1200px] mx-auto bg-surface min-h-screen">
      ...
      <section className="mb-12">
        ...
        <h1 className="text-4xl font-black tracking-tight text-on-surface">Register New Student</h1>
        <p className="text-on-surface-variant mt-2 text-lg">Onboard a student into the institutional database securely.</p>
      </section>
      ...
      <div className="flex items-center gap-4 mb-12 border-b border-outline-variant/15 pb-8 overflow-x-auto">
        {[
          ...
          { num: 2, label: 'Medical & Emergency', icon: Heart },
          { num: 3, label: 'Parent / Guardian', icon: Users },
          { num: 4, label: 'Academic Placement', icon: GraduationCap },
          { num: 5, label: 'Verification', icon: ShieldAlert }
        ].map((s) => (
      ...
      <div className="bg-surface-container-lowest p-10 rounded-3xl border border-outline-variant/10 shadow-sm max-w-3xl">
        ...
        {step === 1 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
            <h3 className="text-xl font-bold tracking-tight text-on-surface border-b border-outline-variant/10 pb-4">Personal Information</h3>
        ...
        {step === 2 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
            <h3 className="text-xl font-bold tracking-tight text-on-surface border-b border-outline-variant/10 pb-4 flex items-center gap-3">
              <Heart className="text-primary w-5 h-5" /> Medical & Emergency
            </h3>
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-2">
            ...
            <div className="flex justify-between pt-6">
              <button onClick={() => setStep(1)} className="px-8 py-4 bg-surface-container-low text-on-surface rounded-xl font-black text-xs uppercase tracking-widest hover:bg-surface-container-high transition-all">Back</button>
        ...
        {step === 3 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
            <h3 className="text-xl font-bold tracking-tight text-on-surface border-b border-outline-variant/10 pb-4 flex items-center gap-3">
              <Users className="text-primary w-5 h-5" /> Parent / Guardian Information
            </h3>
            <div className="grid grid-cols-2 gap-8">
              <div className="space-y-2">
        ...
        {step === 4 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4">
            <h3 className="text-xl font-bold tracking-tight text-on-surface border-b border-outline-variant/10 pb-4">Academic Placement</h3>
            <div className="grid grid-cols-1 gap-8">
              <div className="space-y-2">
                ...
                <select name="section" value={formData.section} onChange={handleChange} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-5 py-4 text-sm font-bold shadow-inner transition-all appearance-none cursor-pointer">
                  ...
                  {isLoadingOptions && <option disabled>Loading...</option>}
                </select>
              </div>
              <div className="space-y-2">
            ...
            <div className="flex justify-between pt-6">
              ...
              <button onClick={() => setStep(5)} disabled={!formData.current_class} className="px-8 py-4 bg-primary text-white rounded-xl font-black text-xs uppercase tracking-widest shadow-lg hover:shadow-xl active:scale-95 transition-all disabled:opacity-50 disabled:cursor-not-allowed">Review Details</button>
            </div>
          </div>
        )}
        ...
        {step === 5 && (
          <div className="space-y-8 animate-in fade-in slide-in-from-right-4 text-on-surface">
            <h3 className="text-xl font-bold tracking-tight border-b border-outline-variant/10 pb-4">Verify Registration</h3>
            <div className="bg-surface-container-low p-8 rounded-3xl border border-outline-variant/20 space-y-4 shadow-inner">
              {/* Photo preview in verification */}
              {photoPreview && (
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest opacity-60">Photo</span>
                  <img src={photoPreview} alt="Student" className="w-12 h-12 rounded-xl object-cover border border-outline-variant/20" />
                </div>
              )}
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest opacity-60">Full Name</span>
                <span className="text-sm font-black">{formData.first_name} {formData.last_name}</span>
              </div>
              <div className="flex justify-between items-center">
                <span className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest opacity-60">DOB / Gender</span>
                <span className="text-sm font-black">{formData.date_of_birth} ({formData.gender === 'M' ? 'Male' : 'Female'})</span>
              </div>
              {formData.blood_group && (
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest opacity-60">Blood Group</span>
                  <span className="text-sm font-black">{formData.blood_group}</span>
                </div>
              )}
              {formData.emergency_contact && (
                <div className="flex justify-between items-center">
                  <span className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest opacity-60">Emergency Contact</span>
              ...
              {formData.parent_name && (
                <div className="flex justify-between items-center pt-3 border-t border-dashed border-outline-variant/20">
                  <span className="text-[10px] font-black text-on-surface-variant uppercase tracking-widest opacity-60">Parent/Guardian</span>
            ...
            <div className="p-6 bg-secondary-container/20 rounded-2xl border border-secondary/10 flex items-start gap-4">
              ...
              <p className="text-xs font-bold text-on-secondary-container/80 leading-relaxed">
                By confirming, this student will be added to the registry as <span className="text-secondary">REGISTERED</span>. They will become <span className="text-secondary">ACTIVE</span> once their initial fees are verified by the treasury.
              </p>
            </div>
