        ...
        <div className="max-w-[1600px] mx-auto space-y-12">
            <div className="flex justify-between items-end">
                <div>
                    <nav className="flex text-xs text-on-secondary-container mb-2 space-x-2">
                        ...
                        <span className="font-semibold text-primary">Regional Monitoring</span>
                    </nav>
                    <h1 className="text-4xl font-extrabold tracking-tight text-on-surface">Regional & Institutional Monitoring</h1>
                    <p className="text-on-surface-variant mt-2">Scope: {dashboardData?.overview.scope || 'National'}</p>
                </div>
            </div>
            ...
            {isLoading ? (
                <div className="bg-white p-12 rounded-xl text-center shadow-sm border border-outline-variant/10">
                    ...
                    <p className="text-on-surface-variant font-medium">Loading regional data...</p>
                </div>
            ) : (
                <>
                    {/* Summary Cards */}
                    <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                        <div className="bg-white p-8 rounded-xl shadow-sm border border-outline-variant/10">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-2">Total Schools</p>
                            <h3 className="text-4xl font-black text-on-surface">{dashboardData?.overview.total_schools || 0}</h3>
                        </div>
                        <div className="bg-white p-8 rounded-xl shadow-sm border border-outline-variant/10">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-2">Total Students</p>
                            <h3 className="text-4xl font-black text-on-surface">{dashboardData?.overview.total_students || 0}</h3>
                        </div>
                        <div className="bg-white p-8 rounded-xl shadow-sm border border-outline-variant/10">
                            <p className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant mb-2">National Attendance</p>
                    ...
                    {regions.length > 0 && (
                        <section className="space-y-6">
                            <div className="flex items-center gap-4">
                                ...
                                <h2 className="text-2xl font-extrabold text-on-surface">Regional Comparison Table</h2>
                            </div>
                                ...
                                <table className="w-full text-left border-collapse">
                                    <thead>
                                        <tr className="bg-slate-50 border-b border-outline-variant/10">
                                            <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-slate-600">Region</th>
                                            <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-slate-600">Schools</th>
                                            <th className="px-6 py-4 text-xs font-black uppercase tracking-widest text-slate-600">Students</th>
                                        </tr>
                                    </thead>
