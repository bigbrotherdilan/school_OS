...
export default function TeacherProfileEdit() {
    ...
    const fetchProfile = async () => {
      try {
        ...
        if (teachers.length > 0) {
          ...
          setProfilePhoto(teacher.user_details?.profile_photo || '');
          setFormData({
            ...
            languages_spoken: (teacher.languages_spoken || []).join(', '),
          });
        }
      } catch (err) {
        addToast(t('Failed to load profile.'), 'error');
      } finally {
        setIsLoading(false);
      }
    };
  ...
  const handlePhotoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!['jpg', 'jpeg', 'png', 'webp', 'gif'].includes(ext || '')) {
      addToast(t('Use JPG, PNG, WebP, or GIF.'), 'error');
      return;
    }
    ...
    try {
      ...
      addToast(err.response?.data?.detail || t('Failed to upload photo.'), 'error');
    } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      addToast(err.response?.data?.detail || t('Failed to update profile.'), 'error');
    } finally {
      setIsSaving(false);
    }
  };
      ...
      <form onSubmit={handleSubmit} className="space-y-10">
        ...
        <div className="bg-surface-container-lowest p-10 rounded-3xl border border-outline-variant/10 shadow-sm">
          <h3 className="text-xl font-bold tracking-tight flex items-center gap-3 border-b border-outline-variant/10 pb-4 mb-6">
            <Camera className="text-primary w-6 h-6" /> {t('Profile Photo')}
          </h3>
          <div className="flex items-center gap-8">
            ...
            <div className="space-y-2">
              ...
              <p className="text-[10px] text-on-surface-variant">{t('JPG, PNG, WebP, or GIF. Max 5MB.')}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-surface-container-lowest p-10 rounded-3xl border border-outline-variant/10 shadow-sm space-y-6">
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Email')}</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Phone')}</label>
        ...
        <div className="bg-surface-container-lowest p-10 rounded-3xl border border-outline-variant/10 shadow-sm space-y-6">
          <h3 className="text-xl font-bold tracking-tight flex items-center gap-3 border-b border-outline-variant/10 pb-4">
            <Briefcase className="text-primary w-6 h-6" /> {t('Professional Details')}
          </h3>
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Qualification')}</label>
              <input type="text" name="qualification" value={formData.qualification} onChange={handleChange} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all" placeholder={t('e.g. MSc Mathematics')} />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Department')}</label>
              <input type="text" name="department" value={formData.department} onChange={handleChange} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all" placeholder={t('e.g. Science')} />
            </div>
          </div>
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Years of Experience')}</label>
              <input type="number" name="years_of_experience" value={formData.years_of_experience} onChange={handleChange} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all" placeholder={t('e.g. 5')} min="0" />
            </div>
            <div className="space-y-2">
          ...
          <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Bio')}</label>
              <textarea name="bio" value={formData.bio} onChange={handleChange} rows={3} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all resize-none" placeholder={t('Tell schools about yourself...')} />
            </div>
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Teaching Philosophy')}</label>
              <textarea name="teaching_philosophy" value={formData.teaching_philosophy} onChange={handleChange} rows={3} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all resize-none" placeholder={t('Describe your teaching approach...')} />
          </div>
        </div>
        ...
        <div className="bg-surface-container-lowest p-10 rounded-3xl border border-outline-variant/10 shadow-sm space-y-6">
          <h3 className="text-xl font-bold tracking-tight flex items-center gap-3 border-b border-outline-variant/10 pb-4">
            <Globe className="text-primary w-6 h-6" /> {t('Marketplace Profile')}
          </h3>
          <p className="text-xs text-on-surface-variant">{t('Make your profile visible on the national teacher marketplace so schools can discover you.')}</p>
          ...
          <div className="flex items-center gap-4 p-4 bg-primary/5 rounded-2xl border border-primary/10">
            ...
            <div>
              <p className="text-sm font-bold">{t('Enable Public Profile')}</p>
              <p className="text-xs text-on-surface-variant">{t('Show your profile in the national teacher directory')}</p>
            </div>
          </div>

          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Subjects Taught (comma-separated)')}</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Specializations (comma-separated)')}</label>
          ...
          <div className="grid grid-cols-2 gap-6">
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Languages (comma-separated)')}</label>
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Hourly Rate (XAF)')}</label>
              <input type="number" name="hourly_rate" value={formData.hourly_rate} onChange={handleChange} className="w-full bg-surface-container-highest border-transparent focus:bg-white focus:border-primary focus:ring-4 focus:ring-primary/5 rounded-xl px-4 py-3 text-sm font-bold shadow-inner transition-all" placeholder={t('Optional')} min="0" />
            </div>
          </div>
          <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Certifications (comma-separated)')}</label>
          ...
          <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-on-surface-variant">{t('Achievements (comma-separated)')}</label>
