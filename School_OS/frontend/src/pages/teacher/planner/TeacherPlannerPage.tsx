  ...
  if (!activeAssignment) {
    return (
      <div className="p-10 text-center text-slate-500 font-medium">
        {t('No active class/subject selected. Pick an assignment to see your lesson plan.')}
      </div>
    );
  }
  ...
  return (
    <div className="space-y-8 pb-12 animate-in fade-in duration-500">
      ...
      <section className="flex flex-col lg:flex-row lg:items-end justify-between gap-5">
        <div>
          <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900">{t('Lesson Planner')}</h2>
        ...
        <div className="flex items-center gap-3">
          <label className="flex items-center gap-2 text-sm font-bold text-slate-600">
            ...
            >
              {terms.length === 0 && <option value="">{t('No terms')}</option>}
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-5 sm:p-6">
        <div className="flex items-center justify-between text-sm font-bold text-slate-600 mb-3">
          <span>{t('Term progress')}</span>
          <span className="text-primary">{t('{{progress}}% covered', { progress })}</span>
        </div>
        ...
        <p className="text-xs text-slate-400 mt-3">
          {t('Mark a week Done when you have covered it — this is your signature of record for work coverage.')}
        </p>
      </div>
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-20 text-center bg-white rounded-3xl border border-slate-100 shadow-sm">
          ...
          <h4 className="text-lg font-bold text-slate-800 mb-2">{t('No lessons planned yet')}</h4>
          <p className="text-sm text-slate-500 max-w-md">
            {t("Your school office hasn't filled in the year plan for this class and subject yet. Once they do, every week will show up here for you to mark as done.")}
          </p>
        </div>
      ) : (
              ...
              <div key={week.id} className={`bg-white rounded-2xl border shadow-sm overflow-hidden transition-all ${
                ...
                <div className="p-5 sm:p-6 flex flex-col sm:flex-row sm:items-start gap-4">
                  <div className={`shrink-0 w-12 h-12 rounded-xl flex flex-col items-center justify-center font-black ${
                    ...
                    <span className="text-[9px] uppercase tracking-wider">{t('Week')}</span>
                  </div>
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2">
                      <h4 className="text-lg font-bold text-slate-900 leading-tight">{week.topic}</h4>
                      {isThisWeek && (
                        <span className="text-[10px] font-bold uppercase text-primary bg-primary/10 px-2 py-0.5 rounded-full">{t('This week')}</span>
                      )}
                      {week.status === 'taught' ? (
                        <span className="text-[10px] font-bold uppercase text-emerald-600 bg-emerald-500/10 px-2 py-0.5 rounded-full flex items-center gap-1">
                          <span className="material-symbols-outlined text-xs">verified</span> {t('Done')}
                        </span>
                      ) : (
                        <span className="text-[10px] font-bold uppercase text-slate-400 bg-slate-100 px-2 py-0.5 rounded-full">{t('Planned')}</span>
                      )}
                    </div>
                    <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-3 text-sm">
                      {week.objectives && <Field icon="outbound" label={t('Objectives')} value={week.objectives} />}
                      {week.expected_outcome && <Field icon="flag" label={t('Expected outcome')} value={week.expected_outcome} />}
                      {week.essential_knowledge && <Field icon="lightbulb" label={t('Essential knowledge')} value={week.essential_knowledge} />}
                      {week.homework && <Field icon="home_work" label={t('Homework')} value={week.homework} />}
                    </div>
                    {week.status === 'taught' && week.teacher_name && (
                      <p className="mt-3 text-xs text-slate-400 flex items-center gap-1">
                        <span className="material-symbols-outlined text-sm">how_to_reg</span>
                        {t('Recorded by {{name}} on {{date}}', { name: week.teacher_name, date: week.taught_at ? new Date(week.taught_at).toLocaleString() : '' })}
                      </p>
                    )}
                    <div className="mt-4">
                      <label className="text-[10px] font-black uppercase tracking-widest text-slate-400 flex items-center gap-1 mb-1.5">
                        <span className="material-symbols-outlined text-sm">edit_note</span> {t('My teaching notes')}
                      </label>
                      <textarea
                        value={week.notes}
                        onChange={(e) => handleNotesChange(week.id, e.target.value)}
                        rows={2}
                        placeholder={t('What actually happened in class, anything to remember...')}
                  ...
                  <div className="sm:ml-auto">
                    {week.status === 'taught' ? (
                      ...
                      >
                        <span className="material-symbols-outlined text-sm">undo</span> {t('Undo')}
                      </button>
                    ) : (
                      <button
