...
export default function TeacherLogbookPage() {
  ...
  const handleFinalize = async () => {
    try {
      ...
      setIsLogged(true);
      addToast(
        'Logbook entry signed and securely archived.',
        'success'
      );
    } catch {
      addToast('Failed to archive logbook entry.', 'error');
    } finally {
      setIsSigning(false);
    }
  };
    ...
    <div className="space-y-8 sm:space-y-12 pb-12 animate-in fade-in duration-500">
      ...
      <section className="space-y-4 sm:space-y-6">
        <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-3">
          <div>
            <h2 className="text-2xl sm:text-3xl font-bold tracking-tight text-slate-900">Academic Journey</h2>
            <p className="text-on-surface-variant text-sm mt-1">Timeline & Compliance History</p>
          </div>
          <button onClick={() => addToast('Archives dashboard coming in Phase 7.', 'info')} className="flex items-center gap-2 text-primary font-bold text-sm hover:underline min-h-[44px]">
            View Historical Archives <span className="material-symbols-outlined text-sm">history</span>
          </button>
        </div>
        ...
        <div className="bg-white p-4 sm:p-8 rounded-2xl shadow-sm border border-slate-100 overflow-x-auto">
          <div className="flex items-center justify-between gap-2 sm:gap-4 min-w-[300px]">
            {dayAbbr.slice(1, 5).map((day) => {
              ...
              return (
                <div key={day} className={`flex flex-col items-center gap-2 sm:gap-3 flex-1 ${isPast ? 'opacity-60' : ''}`}>
                  <span className="text-[10px] sm:text-[11px] font-bold text-slate-400 uppercase tracking-widest">{day}</span>
            ...
            <div className="flex flex-col items-center gap-2 sm:gap-3 flex-1 relative group cursor-pointer transition-all">
              <span className={`text-[10px] sm:text-[11px] font-bold uppercase tracking-widest transition-colors ${isLogged ? 'text-secondary' : 'text-primary'}`}>Today ({todayAbbr})</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-10 items-start">
        ...
        <section className="lg:col-span-5 space-y-4 sm:space-y-6">
          <h3 className="text-sm font-black uppercase tracking-widest text-slate-400">Current Session Context</h3>
            ...
            {currentClass ? (
              <div className="space-y-6 relative z-10">
                <div className="flex justify-between items-start">
                  <div>
                    <p className="text-primary font-bold text-sm uppercase tracking-tighter">Live Session</p>
                ...
                <div className="pt-6 border-t border-slate-50">
                  ...
                  >
                    {isSigning ? 'Signing...' : (isLogged ? 'Securely Artifacted' : 'Sign & Finalize Result')}
              ...
              <div className="py-12 flex flex-col items-center justify-center text-center space-y-3">
                ...
                <p className="text-slate-400 text-sm font-medium">No class currently in session</p>
                <button onClick={() => navigate('/teacher/timetable')} className="text-primary text-xs font-bold uppercase tracking-widest underline underline-offset-4">Browse Schedule</button>
              </div>
            )}
          </div>
          ...
          {isLogged && signatureData && (
            <div className="bg-surface-container-low/50 p-6 rounded-3xl border-2 border-dashed border-secondary/30 flex items-center gap-6 animate-in zoom-in-95 duration-700">
               ...
               <div>
                  <p className="text-[10px] font-black text-secondary uppercase tracking-[0.2em] mb-1">Digital Identity Verified</p>
        ...
        <section className="lg:col-span-7 space-y-4 sm:space-y-6">
          <div className="flex justify-between items-center">
            <h3 className="text-sm font-black uppercase tracking-widest text-slate-400">Documentation Details</h3>
            {isLogged && (
               <span className="text-[10px] font-black text-secondary bg-secondary/10 px-2 py-1 rounded-md flex items-center gap-1">
                 <span className="material-symbols-outlined text-xs">lock</span> READ-ONLY ARCHIVE
               </span>
            )}
          </div>
          ...
          <div className={`bg-white p-5 sm:p-8 rounded-2xl shadow-sm border border-slate-100 space-y-6 sm:space-y-8 transition-all duration-500 ${isLogged ? 'opacity-70 pointer-events-none' : ''}`}>
             <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Macro Topic</label>
                <input 
              ...
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Micro Topic</label>
                <input 
                  className="w-full bg-slate-50 border-none rounded-xl p-3 text-sm font-bold text-slate-700 placeholder:text-slate-300 focus:ring-2 focus:ring-primary/20 transition-all" 
                  type="text" 
            ...
            <div className="space-y-2">
              <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Lesson Narrative & Achievement</label>
              <textarea 
                ...
                placeholder="Describe the knowledge transfer and real-world application context..."
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Pedagogical Feedback</label>
                <select 
                ...
                >
                  <option>Most students achieved outcomes</option>
                  <option>Partial achievement - needs revisit</option>
                  <option>Outstanding engagement & result</option>
                  <option>Remediation required</option>
                </select>
              </div>
              <div className="space-y-2">
                <label className="text-[10px] font-black uppercase tracking-widest text-slate-400">Self-Directed Study (Homework)</label>
                <input 
                  className="w-full bg-slate-50 border-none rounded-xl p-3 text-sm font-bold text-slate-700 placeholder:text-slate-300 focus:ring-2 focus:ring-primary/20 transition-all" 
                  type="text" 
                  placeholder="Exercises, Chapters, or Projects..."
            ...
            <div className="pt-6 border-t border-slate-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
               <p className="text-[9px] font-medium text-slate-400 max-w-[200px]">By signing, you certify this as an accurate academic record under the National Education Framework.</p>
               <div className="flex gap-4 w-full sm:w-auto">
                  <button onClick={() => addToast('Draft saved to archive.', 'success')} className="text-slate-400 text-xs font-bold uppercase tracking-widest hover:text-slate-600 min-h-[44px] flex items-center">Draft Archive</button>
                  <button 
                    onClick={handleFinalize}
