...
export default function FeaturesPage() {
  ...
  const problemsT = problems.map(p => ({ ...p, title: t(p.title), desc: t(p.desc) }));
  const comparisonT = comparison.map(c => ({ ...c, feature: t(c.feature), notebook: t(c.notebook), schoolOs: t(c.schoolOs) }));
  const featuresT = features.map(f => ({ ...f, title: t(f.title), desc: t(f.desc) }));
  const faqsT = faqs.map(f => ({ ...f, q: t(f.q), a: t(f.a) }));
    ...
    <div className="bg-surface text-on-surface selection:bg-primary-fixed selection:text-on-primary-fixed">
      ...
      <header className="relative pt-32 pb-20 md:pt-44 md:pb-28 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-6 text-center relative z-10">
          <span className="inline-flex items-center gap-2 px-4 py-1.5 bg-primary/10 rounded-full mb-6">
            ...
            <span className="text-xs font-bold tracking-widest text-primary uppercase">{t('The Escape Plan')}</span>
          </span>
          ...
          <p className="max-w-2xl mx-auto text-lg md:text-xl text-on-surface-variant mb-10 leading-relaxed">
            {t('One platform. Every system. No notebooks. No spreadsheets. No midnight grading.')}
          </p>
          <div className="flex flex-col sm:flex-row justify-center items-center gap-4">
            ...
            >
              {t("See What You're Escaping From")}
            </a>
          </div>
        </div>
      </header>
      ...
      <section id="the-cost" aria-labelledby="problems-heading" className="py-24 bg-surface-container-low">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-error font-bold tracking-widest text-xs uppercase">{t('The Enemy')}</span>
            <h2 id="problems-heading" className="text-4xl md:text-5xl font-bold text-primary tracking-tight mt-2">
              {t('What The Notebook Costs You')}
            </h2>
            <p className="text-on-surface-variant mt-4 max-w-xl mx-auto">
              {t("Every day your school runs on paper, you're paying for it -- in time, money, and trust.")}
            </p>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
            {problemsT.map((item, i) => (
      ...
      <section aria-labelledby="pick-side-heading" className="py-24">
        <div className="max-w-5xl mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-secondary font-bold tracking-widest text-xs uppercase">{t('Choose Your Side')}</span>
            <h2 id="pick-side-heading" className="text-4xl md:text-5xl font-bold text-primary tracking-tight mt-2">
              {t('The Notebook vs. School OS')}
            </h2>
            <p className="text-on-surface-variant mt-4 max-w-xl mx-auto">
              {t('Every school uses one of these. The question is: which one are you still using?')}
            </p>
          </div>
          ...
          <div className="hidden md:block bg-white rounded-3xl border border-outline-variant/20 shadow-lg overflow-hidden">
            <div className="grid grid-cols-3 border-b border-outline-variant/20">
              ...
              <div className="p-5 text-center bg-error/5 border-x border-outline-variant/20">
                <div className="flex items-center justify-center gap-2">
                  ...
                  <span className="font-bold text-error text-lg">{t('The Notebook')}</span>
                </div>
              </div>
            ...
            {comparisonT.map((row, i) => (
              <div key={i} className={`grid grid-cols-3 ${i < comparisonT.length - 1 ? 'border-b border-outline-variant/10' : ''}`}>
              ...
              <div key={i} className="bg-white rounded-2xl border border-outline-variant/20 shadow-sm overflow-hidden">
                ...
                <div className="p-4 space-y-3">
                  <div className="flex items-start gap-3">
                    ...
                    <div>
                      <span className="text-[10px] font-bold uppercase tracking-widest text-error/70 block mb-0.5">{t('The Notebook')}</span>
                      <span className="text-sm text-on-surface-variant">{row.notebook}</span>
                    </div>
                  </div>
                  <div className="flex items-start gap-3">
        ...
        <div className="max-w-7xl mx-auto px-6">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-6">
            <div className="max-w-xl">
              <span className="text-secondary font-bold tracking-widest text-xs uppercase">{t('The Escape Plan')}</span>
            ...
            <Link to="/schools" className="text-primary font-bold flex items-center gap-2 hover:gap-4 transition-all">
              {t('Explore Schools')} <span className="material-symbols-outlined">arrow_forward</span>
            </Link>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 md:gap-6">
            <article className="md:col-span-2 bg-primary p-10 rounded-3xl text-white relative overflow-hidden flex flex-col justify-between min-h-[340px]">
              <div className="relative z-10">
                <span className="material-symbols-outlined text-5xl mb-4">{featuresT[0].icon}</span>
                <h3 className="text-3xl font-bold mb-3">{featuresT[0].title}</h3>
                <p className="opacity-80 text-base max-w-md leading-relaxed">{featuresT[0].desc}</p>
              </div>
              ...
              <Link to="/find-teachers" className="relative z-10 self-start text-white flex items-center gap-2 mt-6 hover:gap-4 transition-all font-semibold">
                {t('Explore Marketplace')} <span className="material-symbols-outlined">arrow_forward</span>
              </Link>
            </article>

            <div className="flex flex-col gap-4 md:gap-6">
              {featuresT.slice(1, 3).map((f, i) => (
      ...
      <section aria-labelledby="how-it-works-heading" className="py-24">
        <div className="max-w-7xl mx-auto px-6">
          <div className="text-center mb-16">
            <span className="text-secondary font-bold tracking-widest text-xs uppercase">{t("5 Minutes. That's It.")}</span>
            <h2 id="how-it-works-heading" className="text-4xl md:text-5xl font-bold text-primary tracking-tight mt-2">
              {t('Escape the Notebook in 3 Steps')}
            </h2>
            <p className="text-on-surface-variant mt-4 max-w-xl mx-auto">
              {t('No IT department. No training manual. If you can send a WhatsApp, you can do this.')}
            </p>
          </div>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              { step: '01', icon: 'person_add', title: t('Add Your School'), desc: t('Type your school name. Upload a logo if you want. Takes 60 seconds. Done.') },
              { step: '02', icon: 'group_add', title: t('Add Students & Classes'), desc: t('One class, a few students. Or import your whole school from Excel. Your choice.') },
              { step: '03', icon: 'rocket_launch', title: t('Generate Your First Report Card'), desc: t('Click one button. Watch a beautiful PDF appear. That\'s the moment you never go back to the notebook.') },
            ].map((item, i) => (
...
<div className="text-center mb-16">
        <span className="text-secondary font-bold tracking-widest text-xs uppercase">{t('Still Thinking?')}</span>
        <h2 id="faq-heading" className="text-4xl md:text-5xl font-bold text-primary tracking-tight mt-2">
          {t('Questions We Get a Lot')}
        </h2>
      </div>
...
<h2 id="cta-heading" className="text-4xl md:text-5xl font-bold mb-6 relative z-10 tracking-tight">
              {t('The Notebook Era Is Over')}
            </h2>
            <p className="text-lg opacity-80 mb-10 max-w-xl mx-auto relative z-10">
              {t('500+ schools already made the switch. They stopped losing records, stopped grading at midnight, and stopped guessing who paid. Your turn.')}
            </p>
