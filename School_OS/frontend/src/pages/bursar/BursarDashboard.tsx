...
export default function BursarDashboard() {
  ...
  const userName = user?.first_name ?? user?.full_name?.split(' ')[0] ?? t('Bursar');
    ...
    <div className="p-4 lg:p-10 space-y-8 max-w-[1400px] mx-auto animate-in fade-in duration-500">
      ...
      <div>
        <div className="flex items-center gap-2 mb-2">
          ...
          <span className="text-emerald-600 font-bold tracking-widest text-xs uppercase">{t('Bursar Portal')}</span>
        </div>
        <h1 className="text-3xl sm:text-4xl font-bold tracking-tight text-on-surface">
          {t('Welcome back,')} <span className="text-emerald-600">{userName}</span>
        </h1>
        <p className="text-on-surface-variant mt-1 text-sm">{t("Here's your financial overview at a glance.")}</p>
      </div>
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          >
            ...
            <p className="text-[11px] text-on-surface-variant mt-1.5">{t('{{rate}}% of {{amount}}', { rate: collectionRate, amount: formatCFA(summary.total_expected) })}</p>
          </button>
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant/60">{t('Outstanding')}</span>
            ...
            <p className="text-[11px] text-on-surface-variant mt-1.5">{t('{{unpaid}} of {{total}} fees unpaid', { unpaid: summary.unpaid_count, total: summary.total_invoices })}</p>
          </button>
          ...
          <div className="bg-surface-container-lowest p-5 rounded-2xl border border-outline-variant/10">
            ...
            <p className="text-[11px] text-on-surface-variant mt-1.5">{t('Real-time daily total')}</p>
          </div>

          {/* Net Balance */}
          <div className="bg-surface-container-lowest p-5 rounded-2xl border border-outline-variant/10">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant/60">{t('Net Balance')}</span>
            ...
            <p className="text-[11px] text-on-surface-variant mt-1.5">{t('{{amount}} in expenses', { amount: formatCFA(summary.total_expenses) })}</p>
          </div>
        </div>
      )}
      ...
      {/* ─── Quick Actions ─── */}
      <section>
        <h3 className="text-sm font-bold text-on-surface mb-3">{t('Quick Actions')}</h3>
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            ...
            >
              ...
              <span className="text-xs font-bold text-on-surface group-hover:text-primary transition-colors">{t(a.label)}</span>
            </button>
          ))}
        </div>
      </section>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
        ...
        <div className="lg:col-span-3 bg-surface-container-lowest rounded-2xl border border-outline-variant/10 overflow-hidden">
          <div className="px-5 py-4 border-b border-outline-variant/10 flex items-center justify-between">
            <h3 className="text-sm font-bold text-on-surface">{t('Recent Payments')}</h3>
            <button onClick={() => navigate('/bursar/transactions/new')} className="text-xs font-bold text-emerald-600 hover:underline">{t('+ Record')}</button>
          </div>
          {transactions.length === 0 ? (
            <div className="p-12 text-center">
              <span className="material-symbols-outlined text-4xl text-on-surface-variant/30 mb-2 block">receipt_long</span>
              <p className="text-on-surface-variant text-sm font-medium">{t('No payments recorded yet.')}</p>
            </div>
          ) : (
            <table className="w-full text-left">
              <thead>
                <tr className="text-[10px] font-bold uppercase tracking-widest text-on-surface-variant/50 bg-surface-container-low">
                  <th className="px-5 py-2.5">{t('Receipt')}</th>
                  <th className="px-5 py-2.5">{t('Student')}</th>
                  <th className="px-5 py-2.5 text-right">{t('Amount')}</th>
                  <th className="px-5 py-2.5 hidden sm:table-cell">{t('Method')}</th>
                  <th className="px-5 py-2.5 hidden sm:table-cell">{t('Date')}</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-outline-variant/5">
                {transactions.map((tx) => (
                  <tr key={tx.id} className="hover:bg-surface-container-low/50 transition-colors">
                    ...
                    <td className="px-5 py-3 text-sm font-bold text-emerald-600 text-right">{t('CFA')} {tx.amount.toLocaleString()}</td>
        ...
        <div className="lg:col-span-2 bg-surface-container-lowest rounded-2xl border border-outline-variant/10 overflow-hidden">
          <div className="px-5 py-4 border-b border-outline-variant/10">
            <h3 className="text-sm font-bold text-on-surface">{t('Recent Activity')}</h3>
          </div>
          {auditLogs.length === 0 ? (
            <div className="p-12 text-center">
              ...
              {auditLogs.map((log) => (
                <div key={log.id} className="px-5 py-3 flex items-start gap-3 hover:bg-surface-container/20 transition-colors">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="text-[11px] text-on-surface-variant mt-0.5">{log.user_name} · {timeAgo(log.created_at, t)}</p>
                  </div>
                </div>
              ))}
            </div>
