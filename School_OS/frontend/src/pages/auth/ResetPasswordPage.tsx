...
export default function ResetPasswordPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSuccess(true);
      addToast(t('Password reset successfully.'), 'success');
    } catch (error: any) {
      const detail = error.response?.data?.detail || t('Invalid or expired reset link.');
      ...
      <div className="min-h-screen bg-slate-100 flex flex-col justify-center items-center py-12 sm:px-6 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
          <div className="bg-white py-10 px-6 sm:px-10 rounded-3xl shadow-xl border border-slate-200 text-center space-y-6">
            ...
            <h2 className="text-2xl font-extrabold text-slate-900">{t('Password Reset Complete')}</h2>
            <p className="text-sm text-slate-500">
              {t('Your password has been updated. You can now log in with your new password.')}
            </p>
            <Link
              to="/login"
  ...
  return (
    <div className="min-h-screen bg-slate-100 flex flex-col justify-center items-center py-12 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10 text-center mb-8">
        ...
        <h2 className="text-3xl font-extrabold text-slate-900 tracking-tight">{t('Set New Password')}</h2>
        <p className="mt-2 text-sm text-slate-500">{t('Choose a strong password for your account')}</p>
      </div>
      ...
      <div className="sm:mx-auto sm:w-full sm:max-w-md relative z-10">
        <div className="bg-white py-10 px-6 sm:px-10 rounded-3xl shadow-xl border border-slate-200 relative overflow-hidden">
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            <div>
              <label className="block text-sm font-semibold text-slate-700 mb-1">{t('New Password')}</label>
            ...
            >
              {isSubmitting ? <span className="material-symbols-outlined animate-spin">progress_activity</span> : t('Reset Password')}
            </button>
          </form>
        </div>
